...
async def get_sign_in_results(api_id, api_hash, code, phone_number, password, session_name, loop=None, force_sms=False):
...
async def _get_client(loop=None):
    ...
    # Modern Telethon no longer needs an explicit loop passed in
    client = TelegramClient(session_name, api_id, api_hash)
    await client.connect()
...
def _telethon_msg_to_mymessage(msg, target_id, your_name, target_name):
    ...
    Returns:
        MyMessage obj.
    """
    # Build robust media flags using document attributes
    document = getattr(msg, "document", None)
    attributes = list(getattr(document, "attributes", []) or [])

    has_sticker = any(isinstance(a, DocumentAttributeSticker) for a in attributes)
    has_voice = any(isinstance(a, DocumentAttributeAudio) and getattr(a, "voice", False) for a in attributes)
    has_audio = any(isinstance(a, DocumentAttributeAudio) and not getattr(a, "voice", False) for a in attributes)
    has_video_message = any(isinstance(a, DocumentAttributeVideo) and getattr(a, "round_message", False) for a in attributes)
    has_video_file = any(isinstance(a, DocumentAttributeVideo) and not getattr(a, "round_message", False) for a in attributes)

    # Try to append sticker's emoji to text, if present
    sticker_emoji = ""
    for a in attributes:
        if isinstance(a, DocumentAttributeSticker) and getattr(a, "alt", None):
            sticker_emoji = a.alt
            break

    return MyMessage(
        (msg.message or "") + sticker_emoji,
        msg.date.replace(tzinfo=None) + relativedelta(hours=time_offset(msg.date)),
        target_name if getattr(msg, "sender_id", None) == target_id else your_name,

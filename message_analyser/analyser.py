...
async def save_scalar_info(msgs, your_name, target_name, dir_path):
    ...
    with open(dir_path / "scalar_info.csv", 'w', encoding="utf-8") as fp:
        ...
        total_video_msg = len([msg for msg in msgs if getattr(msg, 'has_video_message', False)])
        target_video_msg = len([msg for msg in msgs if getattr(msg, 'has_video_message', False) and msg.author == target_name])
        fp.write(f"Video messages,{total_video_msg},{total_video_msg-target_video_msg},{target_video_msg}\n")
        print_func(f"{'Video messages'.ljust(20)}{total_video_msg:<15d}{total_video_msg-target_video_msg:<15d}{target_video_msg:<15d}")

        total_video_file = len([msg for msg in msgs if getattr(msg, 'has_video_file', False)])
        target_video_file = len([msg for msg in msgs if getattr(msg, 'has_video_file', False) and msg.author == target_name])
        fp.write(f"Video files,{total_video_file},{total_video_file-target_video_file},{target_video_file}\n")
        print_func(f"{'Video files'.ljust(20)}{total_video_file:<15d}{total_video_file-target_video_file:<15d}{target_video_file:<15d}")
...
async def _analyse(msgs, your_name, target_name, words_file, store_msgs=True, store_words=True):
    ...
    results_directory = Path(__file__).resolve().parents[1] / "results" / f"{date}_{your_name}_{target_name}"
    ...
    if store_msgs:
        ...
        storage.store_msgs(results_directory / file_with_msgs, msgs)
    if store_words:
        file_with_words = "words.txt"
        _save_words(msgs, your_name, target_name, results_directory / file_with_words)

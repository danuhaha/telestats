...
def get_non_text_messages_grouped(groups):
    ...
    return [
        ...
        {"groups": [len(list(filter(lambda m: m.has_photo, group))) for group in groups],
         "type": "photo"},
        {"groups": [len(list(filter(lambda m: getattr(m, 'has_video_message', False), group))) for group in groups],
         "type": "video_message"},
        {"groups": [len(list(filter(lambda m: getattr(m, 'has_video_file', False), group))) for group in groups],
         "type": "video_file"},
...
def get_emoji_countered(msgs):
    """Counts all emojis in messages (emoji>=2.x compatible).

    Uses emoji.emoji_list to detect multi-codepoint emojis. Falls back to
    character-based detection if the function is unavailable.
    ...
    for msg in msgs:
        text = msg.text or ""
        try:
            for m in emoji.emoji_list(text):  # emoji>=2.x
                cnt[m["emoji"]] += 1
        except AttributeError:
            # Fallback for older emoji packages
            try:
                emj = getattr(emoji, "UNICODE_EMOJI", {})
            except Exception:
                emj = {}
            for ch in text:
                if ch in emj:
